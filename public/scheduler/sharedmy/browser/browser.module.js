import { Store, AjaxHelper, BrowserHelper, DomSync, DomHelper, EventHelper, Toolbar, Popup, Widget, GlobalEvents, VersionHelper, FunctionHelper } from './schedulerpro.module.js'
...
class ExamplesApp {
  constructor () {
    ...
    const examples = (window.examples || []).map(example => Object.assign(
      {
        fullFolder: me.addTrailingSlash(me.exampleFolder(example)),
        id: me.exampleId(example)
      }, example)
    )
    const storageName = name => `bryntum-${me.product.name}-demo-${name}`
    ...
    const framework = BrowserHelper.queryString.framework || BrowserHelper.getLocalStorageItem('bryntum-framework') || 'js'
    const store = me.examplesStore = new Store({
      data: examples,
      fields: [
        'folder',
        'rootFolder',
  ...
  getDomConfig () {
    ...
    const { theme } = shared
    const productVersion = VersionHelper.getVersion(me.product.name)
    const compareVersion = version => version && productVersion.startsWith(version.match(/^(\d+\.\d+)/)[1])
    const configs = []
    ...
    me.examplesStore.records.forEach(example => {
      if (example.isSpecialRow) {
        ...
        if (groupPaths[group]) {
          ...
          const title = me.isOnline ? '' : 'Path in distribution'
        ...
        const hasPopup = isSF || (example.build && !me.isOnline) || example.offline
        const id = example.id
        const url = isSF ? example.globalUrl : me.fixRTL(example.fullFolder)
